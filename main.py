...
async def main():
    with open("data/current_privates.txt") as f_privates:
        private_lines = f_privates.readlines()
    with open("data/proxies.txt") as f_proxies:
        proxy_lines = f_proxies.readlines()
    wholelines = []
    for i in range(0, len(private_lines)):
        wholeline = private_lines[i].rstrip() + proxy_lines[i]
        wholelines.append(wholeline)
    with open("data/accounts.txt", "w") as f1:
        f1.writelines(wholelines)

    # thread_count = int(input("Введите кол-во потоков: "))
    # thread_count = 10 if thread_count > 10 else thread_count
    thread_count = len(private_lines)
    tasks = []
